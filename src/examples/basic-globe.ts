...
export class BasicGlobeExample {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    // Initialize globe
    this._globe = new Globe(this._canvas, {
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      if (this._globe.sunSystem) {
        ...
        this._globe.sunSystem.startDayNightCycle();
      }
      ...
      if (this._globe.terrainOptimizer) {
        ...
        this._globe.terrainOptimizer.configureLOD({
          maxDistance: 50000,
          transitionZone: 0.2,
        });

        // Set memory limits
        this._globe.terrainOptimizer.configureMemory({
          maxTextureMemory: 512 * 1024 * 1024, // 512MB
          garbageCollectionInterval: 30000,
        });

        // Enable frustum culling
        this._globe.terrainOptimizer.configureFrustumCulling({
          enabled: true,
        });
      }
      ...
      if (this._globe.terrainDetailRenderer) {
        // Configure terrain detail renderer
        this._globe.terrainDetailRenderer.configure({
          enableHeightMaps: true,
          enableNormalMaps: true,
          heightScale: 10000, // 10km max height
          detailScale: 16, // 16x detail multiplier
        });

        // Set texture providers (using default providers)
        this._globe.terrainDetailRenderer.setTextureProviders({
          heightMap: 'https://elevation-tiles-prod.s3.amazonaws.com/terrarium/{z}/{x}/{y}.png',
          normalMap: 'https://elevation-tiles-prod.s3.amazonaws.com/normal/{z}/{x}/{y}.png',
        });
      }
   ...
   */
  public flyTo(longitude: number, latitude: number, altitude = 1000000): void {
   ...
   */
  public async loadArea(west: number, east: number, south: number, north: number, level = 8): Promise<void> {
